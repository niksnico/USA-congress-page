...
async function fetchInfoAndCalculateGlanceTable()
{
    await fetch(url, init)
     ...
     if(apps.rMembers.length!=0) 
      ...
      function calculateTenPercent(array)
      {
      let myMembers=array
      let aux = []
      let aux2=[]
      ...
      let min = reversedArray[tenPercent].missed_votes
      let max = myMembers[tenPercent].missed_votes
      
      for(i=tenPercent;i<myMembers.length;i++)
      {
          {
              if(myMembers[i].missed_votes==max) aux.push(myMembers[i])
          }
          {
              if(reversedArray[i].missed_votes==max) aux2.push(reversedArray[i])
          }
      ...
      [apps.mostEngaged,apps.leastEngaged] = calculateTenPercent(apps.members)
